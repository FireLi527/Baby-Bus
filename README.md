...
- 每门课程维护独立术语库，并提供 HTML 课件和 PPTX 输出；
- 支持多文件分别生成，或综合去重后合并为一套课件；
- 长课件会把各部分资料均匀纳入小节上下文，多来源公平分配上下文预算；生成节点按知识结构自由合并或拆分页，不逐页翻译；
...
- 图片按教学价值选用，不要求逐张覆盖；Agenda、封面、路线图、徽标等装饰资源会被过滤；
- 图片讲解未通过门禁时会逐图单独分析，只回填 `guide/takeaway`；复检仍失败时保留成果并给出提醒，不整节重写；
- 合并各小节后会保守消除“同一资料图、相同结论且没有新增原页依据”的重复页，不影响真正分阶段、分区域的讲解；
- 计划文件记录来源编号、内容哈希、提取字符数和截断状态；
- 显示当前文件和处理阶段；
- 学习中心和生成结果统一保存在项目的 `data/学习资料/`；
- 生成任务由本地服务执行；窗口保持打开时，刷新页面或在页面内切换后仍可继续查看进度；
...
- 知识完整性按理论、公式、推导、例子和结论的实际遗漏判断，不按原页引用计数；图片讲解和排版问题会先自动修正，复检仍不通过时带提醒交付已有成果；
- 浏览器不可用时跳过排版检查；
...
- 关闭窗口会通知本地服务退出；即使生成任务仍在运行也会终止，不会留在后台，重新双击会启动新的本地服务；
- “选择文件夹”继续使用 Windows 系统对话框；
- 配置保存在 `%LOCALAPPDATA%/宝宝巴士/`；学习中心和生成结果统一使用项目的 `data/学习资料/`，每门课程的术语库保存在自己的课程目录中。
